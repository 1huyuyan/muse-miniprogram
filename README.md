...
- **推荐榜 Top 3**：文本模型从候选中生成 Top3 推荐，并校验每条理由与商品自身特征一致，避免"张冠李戴"；识别到 IP 时自动将 IP 周边置顶（榜单守护）
- **送礼话术**：模型生成一句高情商送礼开场白
...
│   ├── result/            # 结果页：画像、推荐榜、话术
│   ├── detail/            # 详情页：商品详情、复制话术、AI 生成贺卡
│   └── settings/          # 设置页：运行时填写接口地址 / Key / 模型
└── assets/goods/          # 商品占位图（g01.png ~ g22.png，g23 起未配图走加载失败占位兜底）
```
...
内置 247 款商品（`g01` ~ `g247`，`utils/goods.js`），每款含名称、品牌、风格标签、参考价、特点描述与占位图；其中 `g01` ~ `g22` 已配图，`g23` 起未配图商品在图片加载失败时自动显示商品名 / 编号占位块。识别到 IP 时自动加权周边 / 联名 / 收藏类商品，IP 名称精确命中高权重入选。
